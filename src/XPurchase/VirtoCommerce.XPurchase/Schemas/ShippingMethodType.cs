...
namespace VirtoCommerce.XPurchase.Schemas
{
    public class ShippingMethodType : ObjectGraphType<ShippingRate>
    {
        public ShippingMethodType()
        {
            ...
            Field<MoneyType>("price", resolve: context => context.Source.Rate.ToMoney(context.Source.Currency));
            Field<MoneyType>("priceWithTax", resolve: context => context.Source.RateWithTax.ToMoney(context.Source.Currency));
            Field<MoneyType>("total", resolve: context => (context.Source.Rate - context.Source.DiscountAmount).ToMoney(context.Source.Currency));
            Field<MoneyType>("totalWithTax", resolve: context => (context.Source.RateWithTax - context.Source.DiscountAmountWithTax).ToMoney(context.Source.Currency));
