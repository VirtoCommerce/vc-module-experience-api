...
namespace VirtoCommerce.XPurchase.Services
{
    public class CartAvailMethodsService : ICartAvailMethodsService
    {
        ...
        public async Task<IEnumerable<CartGiftItem>> GetAvailableGiftsAsync(CartAggregate cartAggr)
        {
            ...
            return giftRewards.Where(reward => productsByIds.ContainsKey(reward.ProductId))
                .Select(reward =>
                {
                    ...
                    var lineItem = _mapper.Map<LineItem>(product);
                    var giftItem = _mapper.Map<CartGiftItem>(lineItem);
                    giftItem.IsAccepted = cartAggr.Cart.Items.Any(li => li.IsGift && li.ProductId == giftItem.ProductId);
                    return giftItem;
                }).ToArray();
        }
